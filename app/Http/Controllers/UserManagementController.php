...
class UserManagementController extends Controller
{
    ...
    public function searchUser(Request $request)
    {
        if (Auth::user()->role_name=='Admin')
        {
            ...
            // search by firstname and status
            if($request->name && $request->status)
            {
                $result = User::where('firstname','LIKE','%'.$request->firstname.'%')
                                ->where('status','LIKE','%'.$request->status.'%')
                                ->get();
            }

            // search by lastname and role name and status
            if($request->name && $request->role_name && $request->status)
            {
                $result = User::where('lastname','LIKE','%'.$request->lastname.'%')
                                ->where('role_name','LIKE','%'.$request->role_name.'%')
                                ->where('status','LIKE','%'.$request->status.'%')
                                ->get();
            }
