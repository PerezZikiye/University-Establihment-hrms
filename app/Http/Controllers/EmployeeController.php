...
class EmployeeController extends Controller
{
    ...
    public function classofserviceIndex()
    {
        $classofservices = DB::table('classofservices')->get();
        return view('form.classofservices',compact('classofservices'));
    }

    /** save record classofservice */
    public function saveRecordClassofservice(Request $request)
    {
        $request->validate([
            'classofservice'        => 'required|string|max:255',
        ]);

        DB::beginTransaction();
        try{

            $classofservice = classofservice::where('classofservice',$request->classofservice)->first();
            if ($classofservice === null)
            {
                $classofservice = new classofservice;
                $classofservice->classofservice = $request->classofservice;
                $classofservice->save();

                DB::commit();
                Toastr::success('Add new classofservice successfully :)','Success');
                return redirect()->route('form/classofservices/page');
            } else {
                DB::rollback();
                Toastr::error('Add new classofservice exits :)','Error');
                return redirect()->back();
            }
        }catch(\Exception $e){
            DB::rollback();
            Toastr::error('Add new classofservice fail :)','Error');
            return redirect()->back();
        }
    }

    /** update record Classofservice */
    public function updateRecordClassofservice(Request $request)
    {
        DB::beginTransaction();
        try{
            // update table Classofservices
            $classofservice = [
                'id'=>$request->id,
                'classofservice'=>$request->classofservice,
            ];
            classofservice::where('id',$request->id)->update($classofservice);
    ...
    /** delete record Class of service */
    public function deleteRecordClassofservice(Request $request)
    {
        try {

            classofservice::destroy($request->id);
            Toastr::success('Classofservice deleted successfully :)','Success');
            return redirect()->back();

        } catch(\Exception $e) {

            DB::rollback();
            Toastr::error('Classofservice delete fail :)','Error');
            return redirect()->back();
        }
    }
    ...
    /** Page Staff Category */
    public function staffcategoryIndex()
    {
        $staffcategories = DB::table('staffcategories')->get();
        return view('form.staffcategories',compact('staffcategories'));
    }

    /** save record Staff Category */
    public function saveRecordStaffcategory(Request $request)
    {
        $request->validate([
            'staffcategory'        => 'required|string|max:255',
        ]);

        DB::beginTransaction();
        try{

            $staffcategory = staffcategory::where('staffcategory',$request->staffcategory)->first();
            if ($staffcategory === null)
            {
                $staffcategory = new staffcategory;
                $staffcategory->staffcategory = $request->staffcategory;
                $staffcategory->save();

                DB::commit();
                Toastr::success('Add new staff category successfully :)','Success');
                return redirect()->route('form/staffcategories/page');
            } else {
                DB::rollback();
                Toastr::error('Add new staffcategory exits :)','Error');
                return redirect()->back();
            }
        }catch(\Exception $e){
            DB::rollback();
            Toastr::error('Add new staffcategory fail :)','Error');
            return redirect()->back();
        }
    }

    /** update record staff category */
    public function updateRecordStaffcategory(Request $request)
    {
        DB::beginTransaction();
        try{
            // update table staff category
            $staffcategory = [
                'id'=>$request->id,
                'staffcategory'=>$request->staffcategory,
            ];
            staffcategory::where('id',$request->id)->update($staffcategory);
    ...
    /** delete record staff category */
    public function deleteRecordStaffcategory(Request $request)
    {
        try {

            staffcategory::destroy($request->id);
            Toastr::success('Staff Category deleted successfully :)','Success');
            return redirect()->back();

        } catch(\Exception $e) {

            DB::rollback();
            Toastr::error('Staff Category delete fail :)','Error');
            return redirect()->back();
        }
    }
    /** END Staff Category */
    ...
    public function unitIndex()
    {
        ...
        return view('form.units',compact('units'));
    }

    /** save record Unit */
    public function saveRecordUnit(Request $request)
    {
        $request->validate([
        ...
        try{
            ...
            $unit = unit::where('unit',$request->unit)->first();
            if ($unit === null)
            {
                $unit = new unit;
                ...
                DB::commit();
                Toastr::success('Add new unit successfully :)','Success');
                return redirect()->route('form/units/page');
            } else {
                DB::rollback();
                Toastr::error('Add new unit exits :)','Error');
                return redirect()->back();
            }
        }catch(\Exception $e){
            DB::rollback();
            Toastr::error('Add new unit fail :)','Error');
            return redirect()->back();
        }
    }

    /** update record unit */
    public function updateRecordUnit(Request $request)
    {
        DB::beginTransaction();
        try{
            // update table unit
            ...
            unit::where('id',$request->id)->update($unit);
    ...
    /** delete record unit */
    public function deleteRecordUnit(Request $request)
    {
        try {
            ...
            DB::rollback();
            Toastr::error('Unit delete fail :)','Error');
            return redirect()->back();
        }
    }
    /** END Unit */
